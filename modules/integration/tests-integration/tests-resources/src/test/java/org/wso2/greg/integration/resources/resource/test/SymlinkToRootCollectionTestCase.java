...
public class SymlinkToRootCollectionTestCase extends GREGIntegrationBaseTest {
    ...
    public void testRemoveAssociation()
        ...
        String associationType = "association";
        wsRegistryServiceClient.removeAssociation(SYMLINK_LOC + SYMLINK_NAME, ASSOCIATION_PATH, associationType);
        AssociationTreeBean aTreeBean =
                relationServiceClient.getAssociationTree(SYMLINK_LOC + SYMLINK_NAME, "association");

        assertFalse(aTreeBean.getAssociationTree().contains(ASSOCIATION_PATH),
                "Remove Association failure");
    }

    @Test(groups = "wso2.greg", dependsOnMethods = "testRemoveAssociation")
    public void testAddLifeCycleToResource()
            throws IOException, LifeCycleManagementServiceExceptionException, InterruptedException, RegistryException {
